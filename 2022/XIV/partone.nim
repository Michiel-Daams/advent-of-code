...
for i in 0 .. input.high:
    ...
    for c in cs:
        ...
        if parsedCoord[1] > yHigh: yHigh = parsedCoord[1]
...
var infiniteVoid = false
var grainCount = 0
while not infiniteVoid: # drop in sand from 500, 0
    var grainPos = (x: sandOrigin, y:0)
    while true:
        if cave.high < grainPos.y + 1: # sand moves out of bounds
            infiniteVoid = true
            break
        elif cave[grainPos.y + 1][grainPos.x] == '.': # if sand can move down straight by one, do this
            grainPos = (grainPos.x, grainPos.y + 1)
            continue
        elif grainPos.x - 1 < 0: # sand moves out of bounds
            infiniteVoid = true
            break
        elif cave[grainPos.y + 1][grainPos.x - 1] == '.': # if sand can move down + left by one, do this
            grainPos = (grainPos.x - 1, grainPos.y + 1)
            continue
        elif grainPos.x + 1 > cave[grainPos.y + 1].high: # sand moves out of bounds
            infiniteVoid = true
            break
        elif cave[grainPos.y + 1][grainPos.x + 1] == '.': # if sand can move down + right by one, do this
            grainPos = (grainPos.x + 1, grainPos.y + 1)
            continue
        else:
            break
